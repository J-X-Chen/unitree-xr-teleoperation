...
| `--startup-max-joint-delta` | 按 **r** 后首个 IK 目标的最大单关节允许跳变量，超过则报错退出；设置为 `<=0` 可关闭检查 |                          弧度值                              |       `1.2`       |
| `--startup-transition-time` | 首个 IK 目标通过检查后，从当前关节位置缓慢过渡到该目标的时间 |                           秒数                               |       `3.0`       |
| `--ready-transition-time` | 程序启动后、等待按 **r** 前，手臂缓慢移动到机器人初始位的时间 |                           秒数                               |       `5.0`       |
| `--shutdown-transition-time` | 退出程序时，从当前关节位置缓慢回到初始位的时间 |                           秒数                               |       `8.0`       |
...
| `--skip-ready-pose` | 【跳过启动预备位】<br />程序启动后不自动缓慢移动手臂到机器人初始位，而是保持当前手臂姿态等待按 **r**。 |
