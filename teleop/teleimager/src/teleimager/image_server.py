...
class CameraFinder:
    ...
    def get_any_uid_by_vpath(self, video_path):
    ...
    def get_vpath_by_ppath(self, physical_path, require_rgb=True):
        base = "/sys/class/video4linux/"
        matches = []
        for v in os.listdir(base):
            ...
            if sys_path == physical_path:
                vpath = f"/dev/{v}"
                if not require_rgb or self._is_like_rgb(vpath):
                    matches.append(vpath)
        if not matches:
            return None
        if len(matches) > 1:
...
class OpenCVCamera(BaseCamera):
    def __init__(self, cam_topic, video_path, img_shape, fps, 
        ...
        if len(self._fourcc) != 4:
            raise ValueError(f"[OpenCVCamera] fourcc must be 4 characters, got {self._fourcc!r}")
    ...
    def __init__(self, cam_config, realsense_enable=False, camera_finder_verbose=False, isaacsim_enable=False):
        ...
        try:
            ...
            for cam_topic, cam_cfg in self._cam_config.items():
                ...
                uvc_format = cam_cfg.get("uvc_format", fourcc)
                allow_non_rgb = cam_cfg.get("allow_non_rgb", False)
                force_video_id = cam_cfg.get("force_video_id", False)
                ...
                if cam_type in ("opencv_direct", "opencv_raw", "opencv_depth"):
                    ...
                    if physical_path is not None:
                        vpath = self._cam_finder.get_vpath_by_ppath(physical_path, require_rgb=not allow_non_rgb)
                        if vpath is None:
                            self._cameras[cam_topic] = None
                    ...
                    uid = None
                    if force_video_id:
                        uid = self._cam_finder.get_any_uid_by_vpath(video_path)
                        if uid is None:
                            self._cameras[cam_topic] = None
