...
class Dex3_1_Controller:
    def __init__(self, left_hand_array_in, right_hand_array_in, dual_hand_data_lock = None, dual_hand_state_array_out = None,
        ...
        direct_q_target_array_in: [input] Optional direct left(7), right(7) hand motor target.
                                  If provided, control_process bypasses hand retargeting.
        ...
        if direct_q_target_array_in is not None:
    ...
    def control_process(self, left_hand_array_in, right_hand_array_in, left_hand_state_array, right_hand_state_array,
        ...
        left_q_target  = np.array(left_hand_state_array[:], dtype=float)
        right_q_target = np.array(right_hand_state_array[:], dtype=float)
        max_delta_q = 0.03
        ...
        try:
            while self.running:
                ...
                desired_left_q_target = left_q_target.copy()
                desired_right_q_target = right_q_target.copy()

                if direct_q_target_array_in is not None:
                    with direct_q_target_array_in.get_lock():
                        direct_q_target = np.array(direct_q_target_array_in[:]).copy()
                    if direct_q_target.shape[0] == Dex3_Num_Motors * 2 and np.all(np.isfinite(direct_q_target)):
                        desired_left_q_target = direct_q_target[:Dex3_Num_Motors]
                        desired_right_q_target = direct_q_target[Dex3_Num_Motors:]
                    else:
                        logger_mp.warning("[Dex3_1_Controller] Ignoring invalid direct q target.")
                else:
                    ...
                    if not np.all(right_hand_data == 0.0) and not np.all(left_hand_data[4] == np.array([-1.13, 0.3, 0.15])): # if hand data has been initialized.
                        ...
                        desired_left_q_target  = self.hand_retargeting.left_retargeting.retarget(ref_left_value)[self.hand_retargeting.right_dex_retargeting_to_hardware]
                        desired_right_q_target = self.hand_retargeting.right_retargeting.retarget(ref_right_value)[self.hand_retargeting.right_dex_retargeting_to_hardware]

                if not np.all(np.isfinite(desired_left_q_target)) or not np.all(np.isfinite(desired_right_q_target)):
                    logger_mp.warning("[Dex3_1_Controller] Ignoring non-finite q target.")
                    desired_left_q_target = left_q_target.copy()
                    desired_right_q_target = right_q_target.copy()

                left_q_target = np.clip(desired_left_q_target, left_q_target - max_delta_q, left_q_target + max_delta_q)
                right_q_target = np.clip(desired_right_q_target, right_q_target - max_delta_q, right_q_target + max_delta_q)
